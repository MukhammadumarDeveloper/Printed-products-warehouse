...
public class CsvLineParser<T> {
    ...
    public T parse(String[] csvLine){
        if(csvLine.length != extractors.size()){
            throw new IllegalArgumentException("Csv file line is not valid. Expected " + extractors.size()
    ...
    public static record Extractor<V,T>(BiConsumer<T,V>setter, Function<String,V> converter){
        ...
        public  static  <T> Extractor<Long, T> forLong(BiConsumer<T, Long> setter){
            return new Extractor<>(setter, Long::parseLong);
        }

        public static <T> Extractor<Double, T> forDouble(BiConsumer<T, Double> setter){
            return new Extractor<>(setter, Double::parseDouble);
        }
    }
}
